...
class PageTemplateResponseTests(unittest.TestCase):
    ...
    def test_route_collector_uses_third_argument_as_reference_description(self):
        ...
        try:
            ...
            self.assertIn(
                "No JSON request body is expected.",
                reference["rest_routes"][0]["methods"][0]["input_schema"]["properties"]["body"]["description"],
            )
    ...
    def test_develop_template_renders_reference_dict(self):
        ...
        try:
            ...
            self.assertIsNotNone(response)
            html = response.body.decode("utf-8")
            self.assertIn('<header class="fixed-top"', html)
            self.assertIn("RCT Monitor Protocol Reference", html)
            self.assertIn("<h1>Overview</h1>", html)
            self.assertNotIn("<h1>Monitor Protocol Reference</h1>", html)
            self.assertIn("--rct-develop-navbar-height: 49px", html)
            self.assertIn("padding-top: var(--rct-develop-navbar-height)", html)
            self.assertIn("top: var(--rct-develop-navbar-height)", html)
            self.assertIn("Common JSON Schemas", html)
            self.assertIn('data-spy-target="common-schemas"', html)
            self.assertIn('id="common-schemas" data-spy-section="common-schemas"', html)
            self.assertIn("rct-protocol-index-child", html)
            self.assertIn("Status State", html)
            self.assertIn("Full monitor state object returned as REST metadata state", html)
            ...
            self.assertIn("No JSON request body is expected.", html)
            self.assertIn("Whether the command was accepted.", html)
            self.assertNotIn("rct-schema-description-list", html)
            self.assertIn("rct-protocol-schema-block", html)
            ...
            self.assertIn("rct-protocol-route-divider", html)
