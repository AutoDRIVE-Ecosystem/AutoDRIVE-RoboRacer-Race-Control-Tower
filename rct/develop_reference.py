...
ANY_JSON_SCHEMA: dict[str, Any] = {"description": "Any JSON value."}
...
def ws_envelope_schema(
    ...
    *,
    ...
    return object_schema(
        {
            "event": string_schema(enum=(event,), description="Monitor WebSocket event name."),
            "timestamp": string_schema(description="UTC ISO-8601 timestamp generated by RCT."),
            **properties,
        },
        required=("event", "timestamp", *required),
        description=description,
    )


DEVKIT_ENDPOINT_SCHEMA = object_schema(
    {
        "vehicle_id": INTEGER_SCHEMA,
        "host": string_schema(description="DevKit bridge host name or IP address."),
        "hostname": string_schema(description="Alias for host."),
        "port": INTEGER_SCHEMA,
    },
    required=("vehicle_id",),
    description="DevKit endpoint command payload. Configure commands require host or hostname plus port.",
)


MONITOR_WS_MESSAGES: tuple[dict[str, Any], ...] = (
    {
        "kind": "Server Event",
        "name": "status",
        ...
        "schema": ws_envelope_schema(
            "status",
            {
                "monitor_protocol": MONITOR_PROTOCOL_SCHEMA,
                "trace_lidar_vehicle_ids": array_schema(
                    INTEGER_SCHEMA,
                    description="Vehicles with decoded LiDAR telemetry enabled.",
                ),
                "revision": INTEGER_SCHEMA,
                "simulator_clients": INTEGER_SCHEMA,
                "monitor_clients": INTEGER_SCHEMA,
                ...
                "audit_rule": AUDIT_RULE_SCHEMA,
                "penalty_decision": PENALTY_DECISION_SCHEMA,
                "vehicle_penalties": object_schema(
                    {},
                    additional_properties=INTEGER_SCHEMA,
                    description="Penalty count by vehicle id string.",
                ),
                "race_result": RACE_RESULT_SCHEMA,
                "review_time_seconds": NUMBER_SCHEMA,
                "simulator_socketio_path": string_schema(description="Socket.IO path used by simulator clients."),
            },
            description="Server-to-client status event envelope. Full status includes settings fields.",
        ),
    },
    {
        "kind": "Server Event",
        ...
        "schema": ws_envelope_schema(
            "telemetry",
            {
                "source": string_schema(description="Telemetry source, normally rct-cache."),
                "socketio_event": string_schema(description="Socket.IO event that supplied the telemetry."),
                "race_time_seconds": NUMBER_SCHEMA,
                "review_time_seconds": NUMBER_SCHEMA,
                "vehicles": object_schema(
                    {},
                    additional_properties=object_schema(
                        {},
                        additional_properties=ANY_JSON_SCHEMA,
                        description="Telemetry fields for one vehicle.",
                    ),
                    description="Map from vehicle id string to monitor telemetry fields.",
                ),
            },
            required=("vehicles",),
            description="Server-to-client cached telemetry event envelope.",
        ),
    },
    {
        "kind": "Server Event",
        ...
        "schema": ws_envelope_schema(
            "frame",
            {
                "source": string_schema(description="Bridge traffic source."),
                "target": string_schema(description="Bridge traffic target."),
                "socketio_event": string_schema(description="Socket.IO event name."),
                "vehicle_id": INTEGER_SCHEMA,
                "args": array_schema(ANY_JSON_SCHEMA, description="Socket.IO argument list after JSON-safe encoding."),
            },
            required=("source", "target", "socketio_event", "args"),
            description="Server-to-client bridge observation event envelope.",
        ),
    },
    {
        "kind": "Server Event",
        ...
        "schema": ws_envelope_schema(
            "accident-logs",
            {
                "source": string_schema(description="Event source, normally rct."),
                "accident_logs": array_schema(ANY_OBJECT_SCHEMA, description="Accident log records newest first."),
            },
            required=("accident_logs",),
            description="Server-to-client accident log list event envelope.",
        ),
    },
    {
        "kind": "Server Event",
        ...
        "schema": ws_envelope_schema(
            ...
            {
                "source": string_schema(description="Event source, normally rct."),
                "audit_log": array_schema(ANY_OBJECT_SCHEMA, description="Full audit log list."),
                "audit_entry": ANY_OBJECT_SCHEMA,
            },
            description="Server-to-client audit event envelope. Contains either audit_log or audit_entry.",
        ),
    },
    {
        "kind": "Server Event",
        ...
        "schema": ws_envelope_schema(
            "penalty-decision",
            {
                "source": string_schema(description="Event source."),
                "active": BOOLEAN_SCHEMA,
                "collision_vehicle_ids": array_schema(INTEGER_SCHEMA, description="Vehicles in the collision decision."),
                "filtered_vehicle_ids": array_schema(INTEGER_SCHEMA, description="Vehicles currently control-filtered."),
                "penalty_vehicle_id": {"type": ["integer", "null"], "description": "Vehicle selected for penalty."},
                "victim_vehicle_id": {"type": ["integer", "null"], "description": "Vehicle selected as victim."},
                "no_decision": BOOLEAN_SCHEMA,
                "review_time_seconds": NUMBER_SCHEMA,
                "release_delay_seconds": NUMBER_SCHEMA,
            },
            description="Server-to-client manual penalty-decision workflow event envelope.",
        ),
    },
    {
        "kind": "Client Command",
        ...
        "schema": object_schema(
            {
                "command": string_schema(enum=("configure-devkits",), description="Command name."),
                "devkits": array_schema(DEVKIT_ENDPOINT_SCHEMA, description="DevKit endpoints to configure."),
            },
            required=("command", "devkits"),
            description="Client-to-server command sent as one WebSocket JSON text frame.",
        ),
    },
    {
        "kind": "Client Command",
        ...
        "schema": object_schema(
            {
                "command": string_schema(enum=("connect-devkit",), description="Command name."),
                "vehicle_id": INTEGER_SCHEMA,
                "host": string_schema(description="Optional replacement DevKit bridge host."),
                "hostname": string_schema(description="Alias for host."),
                "port": INTEGER_SCHEMA,
            },
            required=("command", "vehicle_id"),
            description="Client-to-server command sent as one WebSocket JSON text frame.",
        ),
    },
    {
        ...
        "schema": object_schema(
            {
                "command": string_schema(enum=("disconnect-devkit",), description="Command name."),
                "vehicle_id": INTEGER_SCHEMA,
            },
            required=("command", "vehicle_id"),
            description="Client-to-server command sent as one WebSocket JSON text frame.",
